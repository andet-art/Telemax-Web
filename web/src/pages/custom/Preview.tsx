...
import { ArrowLeft, CheckCircle, ShoppingCart, ShieldCheck } from "lucide-react";
...
export default function Preview() {
  ...
  const draft: CustomDraft | null = useMemo(() => {
    ...
    return stateDraft?.head && stateDraft?.ring && stateDraft?.tail ? stateDraft : readDraftFromSession();
  }, [location.state]);
  ...
  const accent = draft?.accent ?? "gold";
  const accentChip =
    accent === "ember"
  ...
  const onAddToCart = useCallback(() => {
    if (!draft) {
      showToast("No build found. Go back and complete the 3 steps.");
      return;
    }
    if (!pipeName.trim()) {
    ...
    addToCart({
      ...
      type: "custom",
      name: pipeName.trim(),
      price: Number(draft.total || 0),
      quantity: 1,
      currency: draft.currency || "EUR",
      image: resolvePhoto(draft.head.photo), // main image = bowl
      head: draft.head,
      ring: draft.ring,
      tail: draft.tail,
    } as any);
    ...
    navigate("/cart");
  }, [addToCart, draft, navigate, pipeName, showToast, t]);
      ...
      <main className="min-h-screen pt-24 pb-20 bg-black text-white flex items-center justify-center px-6">
        <div className="max-w-lg w-full rounded-3xl border border-white/10 bg-gradient-to-br from-[#0f0b07]/95 to-[#1b120b]/95 p-6">
          ...
          <div className="text-stone-300 mb-6">
            Go back to the builder and complete the 3 steps first.
          </div>
          <button
        ...
        <div className="relative z-10 max-w-6xl mx-auto px-3 sm:px-6">
          ...
          >
            ...
            <p className="text-stone-300 max-w-2xl mx-auto">
              Review the 3-part build, name it, then add it to your cart.
            </p>
          </motion.div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-5">
            ...
            >
              <div className="p-4 sm:p-6 border-b border-white/10">
                <div className="text-sm text-stone-400">3-part build</div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  ...
                  <div className="rounded-2xl border border-white/10 bg-black/25 overflow-hidden">
                    <div className="p-3 border-b border-white/10 flex items-center justify-between">
                      ...
                      <div className="text-xs text-stone-400">{fmtMoney(Number(draft.head.price || 0))}</div>
                    </div>
                    ...
                    <div className="p-3 text-sm text-stone-200">{draft.head.name}</div>
                  </div>
                  ...
                  <div className="rounded-2xl border border-white/10 bg-black/25 overflow-hidden">
                    <div className="p-3 border-b border-white/10 flex items-center justify-between">
                      ...
                      <div className="text-xs text-stone-400">{fmtMoney(Number(draft.ring.price || 0))}</div>
                    </div>
                    <div className="relative aspect-[4/5] bg-black/40">
                      <img
                    ...
                    <div className="p-3 text-sm text-stone-200">{draft.ring.name}</div>
                  </div>
                  ...
                  <div className="rounded-2xl border border-white/10 bg-black/25 overflow-hidden">
                    <div className="p-3 border-b border-white/10 flex items-center justify-between">
                      ...
                      <div className="text-xs text-stone-400">{fmtMoney(Number(draft.tail.price || 0))}</div>
                    </div>
                    <div className="relative aspect-[4/5] bg-black/40">
                      <img
                    ...
                    <div className="p-3 text-sm text-stone-200">{draft.tail.name}</div>
                  </div>
                </div>
                ...
                <div className="mt-5 flex items-center justify-between gap-3">
                  <div>
                    ...
                    <div className="text-2xl font-bold text-[#c9a36a]">
                      {fmtMoney(Number(draft.total || 0), draft.currency || "EUR")}
                    </div>
                  </div>
