...
export default function Tail({ value, onChange, onToast }: TailProps) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        setLoading(true);
        const res = await api.get("/parts", { params: { part_type: "tail" } });
  ...
  const items = useMemo(
    () =>
      tails.map((t) => ({
        ...t,
        _photo: resolvePhoto(t.photo),
      })),
    [tails]
  );

  if (loading) return <div className="text-stone-300">Loading stems…</div>;
  if (items.length === 0)
    return (
      ...
      {items.map((p, idx) => {
        ...
        return (
          <motion.button
            ...
            onClick={() => {
              onChange(p);
              onToast?.("Stem attached. Your pipe is complete.");
            }}
